    ...
    foreach($Loc in $Locs)
    {
      ...
      foreach($Num in $Nums)
      {
        ...
        if($null -eq $Pool)
        {
          ...
          Write-Output "Pool    $Name - Created"
        }
        else
        {
            Write-Output "Pool    $Name"
        }
        ...
        # Resize the pool 
        $CmdText = "& az batch pool resize --resize-timeout `"PT30M`" --target-low-priority-nodes 3 --target-dedicated-nodes 0 --pool-id `"$Name`""
        Invoke-Expression -Verbose -Command $CmdText
        Write-Output "Resized $Name"
      }
    }
  }
  # 